...
function FamilyManagementContent() {
  ...
  const submitChild = async (event: FormEvent) => {
    ...
    await run("child", async () => {
      const payload = {
        ...
        current_grade_level: childGrade ? Number(childGrade) : null,
        school_year: childSchoolYear || null,
      };
  ...
  const startEditChild = (child: Child) => {
    ...
    setChildGrade(child.current_grade_level ? String(child.current_grade_level) : "");
    setChildSchoolYear(child.school_year ?? "");
  };
  ...
  return (
    <section className="family-management-page section-shell">
      ...
      <section className="family-management-card"><div><h2>孩子资料</h2><p>年级由家长设置，不根据生日自动推断；换年级不会清空 Mastery。</p></div><div className="family-child-list">{allChildren.map((child) => <article className={child.is_archived ? "archived" : ""} key={child.id}><div><strong>{child.display_name}</strong><small>{child.nickname || "暂无昵称"} · {child.birth_date} · {child.current_grade_level ? `${child.current_grade_level} 年级` : "启蒙 / 未设置年级"}{child.school_year ? ` · ${child.school_year}` : ""}{child.is_archived ? " · 已归档" : ""}</small></div>{isAdmin ? <div><button className="button button-secondary" onClick={() => startEditChild(child)} type="button">编辑</button><button className="button button-secondary" disabled={working === `archive:${child.id}`} onClick={() => void run(`archive:${child.id}`, async () => { if (child.is_archived) await restoreChild(child.id); else await archiveChild(child.id); await Promise.all([load(), refreshHousehold()]); }, child.is_archived ? "孩子资料已恢复。" : "孩子资料已归档，历史证据未删除。") } type="button">{child.is_archived ? "恢复" : "归档"}</button></div> : null}</article>)}</div>{isAdmin ? <form className="family-child-form" onSubmit={(event) => void submitChild(event)}><h3>{editingChildId ? "编辑孩子资料" : "+ 添加孩子"}</h3><label><span>姓名</span><input maxLength={80} onChange={(event) => setChildName(event.target.value)} required value={childName} /></label><label><span>昵称（可选）</span><input maxLength={80} onChange={(event) => setChildNickname(event.target.value)} value={childNickname} /></label><label><span>出生日期</span><input max={new Date().toISOString().slice(0, 10)} onChange={(event) => setChildBirthDate(event.target.value)} required type="date" value={childBirthDate} /></label><label><span>性别</span><select onChange={(event) => setChildGender(event.target.value as ChildGender | "")} value={childGender}><option value="">暂不填写</option><option value="female">女</option><option value="male">男</option><option value="other">其他</option></select></label><label><span>当前年级</span><select onChange={(event) => setChildGrade(event.target.value)} value={childGrade}><option value="">启蒙 / 暂不设置</option>{Array.from({ length: 9 }, (_, index) => index + 1).map((item) => <option key={item} value={item}>{item} 年级</option>)}</select></label><label><span>学年（可选）</span><input onChange={(event) => setChildSchoolYear(event.target.value)} pattern="\d{4}-\d{4}" placeholder="2026-2027" value={childSchoolYear} /></label><button className="button button-primary" disabled={working === "child"} type="submit">{editingChildId ? "保存修改" : "添加孩子"}</button>{editingChildId ? <button className="button button-secondary" onClick={() => { setEditingChildId(""); setChildName(""); setChildNickname(""); setChildBirthDate(""); setChildGender(""); setChildGrade(""); setChildSchoolYear(""); }} type="button">取消编辑</button> : null}</form> : null}</section>
