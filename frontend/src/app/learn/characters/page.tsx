    ...
    <section className="character-learning-page section-shell">
      ...
      {view === "session" && session ? (
        <section className="learning-workspace assessment-workspace" id={`assessment-session-${session.id}`}>
          <header><div><p className="eyebrow">{SOURCE_LABELS[session.source]}</p><h2>{session.status === "completed" ? "完成啦 🎉" : `${session.completed_items + 1} / ${session.total_items}`}</h2></div></header>
