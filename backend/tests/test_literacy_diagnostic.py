...
async def create_family_and_child(client: httpx.AsyncClient, suffix: str = "") -> tuple[dict, dict]:
    family_response = await client.post("/api/v1/families", json={"name": f"识字检测家庭{suffix}"})
...
def test_representative_positions_cover_all_120_ten_character_strata() -> None:
    ...
    assert all(
        index * 10 <= position < (index + 1) * 10 for index, position in enumerate(positions)
    )
...
async def test_standard_diagnostic_is_resumable_and_only_updates_tested_characters(
    ...
    overview = await client.get(f"/api/v1/children/{child['id']}/literacy-diagnostic/overview")
...
async def test_diagnostic_rejects_hints_and_cross_family_access(
    ...
    async with (
        ...
        assert (
            await outsider.get(f"/api/v1/children/{child['id']}/literacy-diagnostic/overview")
        ).status_code == 404
        assert (
            await outsider.post(f"/api/v1/children/{child['id']}/literacy-diagnostic/start")
        ).status_code == 404

        started = await owner.post(f"/api/v1/children/{child['id']}/literacy-diagnostic/start")
        assert started.status_code == 200
